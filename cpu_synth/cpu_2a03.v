     ...
     if (nreset)
       begin
          ...
          case(cyc_count_control)
            `CYC_COUNT_INCR:   cyc_count <= cyc_count + 'h1;
            `CYC_COUNT_RESET:  cyc_count <= 'b000;
            `CYC_COUNT_SET1:   cyc_count <= 'b001;
            `CYC_COUNT_SET1_IF_NOBRANCH: cyc_count <= do_branch ? (cyc_count + 'b1) : 'b1;
            `CYC_COUNT_SET1_IF_SAMEPAGE: cyc_count <= (page_boundary_crossed) ? (cyc_count + 'b1) : 'b1;
          endcase
